...
from omnisafe.models.actor.cnn_actor import CNNActor
from omnisafe.models.actor.cnn_sac_actor import CNNSACActor
from omnisafe.models.actor.fused_sac_actor import FusedSACActor
...
class ActorBuilder:
    ...
    def build_actor(
        ...
        Currently, we support the following actor types:
            ...
            - ``cnn``: CNN actor for image observations, used in ``DDPG`` and ``TD3``.
            - ``cnn_sac``: CNN actor with stochastic policy for image observations, used in ``SAC``.
        ...
        if actor_type == 'fused_sac':
            return FusedSACActor(
                self._obs_space,
                self._act_space,
                hidden_sizes=self._hidden_sizes,
                activation=self._activation,
        ...
        raise NotImplementedError(
            ...
            f'Available actor types are: gaussian_learning, gaussian_sac, mlp, vae, perturbation, cnn, cnn_sac, fused_sac.',
        )

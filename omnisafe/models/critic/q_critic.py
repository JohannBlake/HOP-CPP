    ...
    def forward(
        ...
        for critic in self.net_lst:
            if self._use_obs_encoder:
                ...
                # Ensure act is [B, act_dim] and obs_encode is [B, feat_dim]
                if act.dim() == 1:
                    act = act.unsqueeze(0)
                # If obs_encode is [B, feat_dim] and act is [B, act_dim], shapes match
                # If obs_encode is [B, feat_dim] and act is [B, act_dim], but B mismatches, try to broadcast
                if obs_encode.shape[0] != act.shape[0]:
                    # Try to broadcast singleton batch
                    if act.shape[0] == 1:
                        act = act.expand(obs_encode.shape[0], -1)
                    elif obs_encode.shape[0] == 1:
                        obs_encode = obs_encode.expand(act.shape[0], -1)
                x = torch.cat([obs_encode, act], dim=-1)
                res.append(torch.squeeze(critic[1](x), -1))
            else:
                # For non-image obs, just concatenate directly
                # If obs is 4D (image), flatten it first
                if obs.dim() > 2:
                ...
                if act.dim() == 1:
                    act = act.unsqueeze(0)
                if obs_flat.shape[0] != act.shape[0]:
                    if act.shape[0] == 1:
                        act = act.expand(obs_flat.shape[0], -1)
                    elif obs_flat.shape[0] == 1:
                        obs_flat = obs_flat.expand(act.shape[0], -1)

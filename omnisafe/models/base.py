...
class Actor(nn.Module, ABC):
    ...
    def __init__(
        ...
        if isinstance(self._obs_space, spaces.Box):
            if len(self._obs_space.shape) == 1:
                self._obs_dim: int = self._obs_space.shape[0]
            elif len(self._obs_space.shape) == 3:
                # For images, not used by CNNActor, but set for compatibility
                self._obs_dim: int = int(np.prod(self._obs_space.shape))
            else:
                raise NotImplementedError(f"Unsupported obs_space shape: {self._obs_space.shape}")
        else:
    ...
    def __init__(
        ...
        if isinstance(self._obs_space, spaces.Box):
            if len(self._obs_space.shape) == 1:
                ...
                # For images, not used by CNNQCritic, but set for compatibility

...
class ActorQCritic(nn.Module):
    ...
    def __init__(
        ...
        # Determine actor/critic types based on fusion flag
        use_fused = getattr(para, 'ablation_study_fuse_lidar_sensor_data_and_image_data', False)
        use_radiation = getattr(para, 'ablation_study_use_radiation_instead_of_lidar', False)
        
        if use_fused and not use_radiation:
            # Use fused actor/critic for lidar+image fusion
            actor_type = 'fused_sac'
            critic_type = 'fused_q'
            print(f"Using fused lidar+image architecture: actor_type={actor_type}, critic_type={critic_type}")
        else:
            # Use configured actor/critic types
            actor_type = model_cfgs.actor_type
            critic_type = getattr(model_cfgs, 'critic_type', 'q')
        ...
        ).build_critic(critic_type=critic_type)

...
class OffPolicyAdapter(OnlineAdapter):
    ...
    def rollout(  # pylint: disable=too-many-locals
        ...
        for _ in range(rollout_step):
            ...
            # Handle Dict observations for cloning
                ...
                if done:
                    if 'final_observation' in info:
                        if isinstance(real_next_obs, dict):
                            # Update each key in the dict observation
                            for k in real_next_obs:
                                if k in info['final_observation']:
                                    real_next_obs[k][idx] = info['final_observation'][k][idx]
                        else:

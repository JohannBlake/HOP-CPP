...
class Unsqueeze(Wrapper):
    ...
    def __init__(self, env: CMDP, device: torch.device) -> None:
        ...
        assert isinstance(self.observation_space, (spaces.Box, spaces.Dict)), (
            f'Observation space must be Box or Dict, got {type(self.observation_space)}'
        )
        self._is_dict_obs = isinstance(self.observation_space, spaces.Dict)

    def _unsqueeze_obs(self, obs: torch.Tensor | dict) -> torch.Tensor | dict:
        """Unsqueeze observation, handling both tensor and dict formats."""
        if self._is_dict_obs:
            return {k: v.unsqueeze(0) for k, v in obs.items()}
        return obs.unsqueeze(0)

    def _squeeze_obs(self, obs: torch.Tensor | dict) -> torch.Tensor | dict:
        """Squeeze observation, handling both tensor and dict formats."""
        if self._is_dict_obs:
            return {k: v.squeeze(0) for k, v in obs.items()}
    ...
    def step(
        ...
        reward, cost, terminated, truncated = (
            x.unsqueeze(0) for x in (reward, cost, terminated, truncated)
        )

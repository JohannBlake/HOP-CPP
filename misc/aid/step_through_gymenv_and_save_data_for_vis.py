    ...
    for run_id in run_ids:
        ...
        if cluster == 'lmu':
            ...
            if para.training_library == 'sb3':
                ...
                # Use different paths based on omnisafe_alg
                if para.omnisafe_alg[:3] == "SAC":
                    base_path = f"/home/stud/blake/git_clones/Simulation_{sweep_id}/.experiments/*/*/torch_save"
                else:
                    base_path = f"/home/stud/blake/git_clones/Simulation_{sweep_id}/misc/logs/runs/*/*/torch_save"
                
                if constrained_model_number == "":
                    print("Using latest model, since no constrained_model_number is specified.")
                    list_cmd = (
                        f'ssh blake@madeira.dbs.ifi.lmu.de "ls {base_path}/epoch-*.pt"'
                    )
                else:
                    print("Using model from epoch ", constrained_model_number)
                    list_cmd = (
                        f'ssh blake@madeira.dbs.ifi.lmu.de "ls {base_path}/epoch-{constrained_model_number}.pt"'
                    )
            ...
            if para.training_library == 'sb3':
                ...
                # Use different paths based on omnisafe_alg
                if para.omnisafe_alg[:3] == "SAC":
                    base_path = f"/dss/dsshome1/0C/di97sog/git_clones/Simulation_{sweep_id}/.experiments/*/*/torch_save"
                else:
                    base_path = f"/dss/dsshome1/0C/di97sog/git_clones/Simulation_{sweep_id}/misc/logs/runs/*/*/torch_save"
                
                if constrained_model_number == "":
                    print("Using latest model, since no constrained_model_number is specified.")
                    list_cmd = (
                        f'ssh -i "{ssh_key}" {remote_user} "ls {base_path}/epoch-*.pt"'
                    )
                else:
                    print("Using model from epoch ", constrained_model_number)
                    list_cmd = (
                        f'ssh -i "{ssh_key}" {remote_user} "ls {base_path}/epoch-{constrained_model_number}.pt"'
                    )
                result = subprocess.run(list_cmd, shell=True, capture_output=True, text=True, encoding='utf-8', check=True)
                files = [f for f in result.stdout.strip().split('\n') if f]
                # Get file with latest creation/modification time
        ...
        # Load actor_type from the cloned repo's config file based on para.omnisafe_alg
        alg_config_path = os.path.join(base_folder, 'omnisafe', 'configs', 'off-policy', f'{para.omnisafe_alg}.yaml')
        with open(alg_config_path, 'r') as f:
            alg_config = yaml.safe_load(f)
        actor_type = alg_config['defaults']['model_cfgs']['actor_type']
        print(f"Using actor_type from config: {actor_type}")
        ...
        obs, _ = env.reset(seed=0 + skip_seed_offset)
        
        for _ in range(num_episodes_to_skip*2):
            print("reset", _)
            obs, _ = env.reset(seed=0 + skip_seed_offset + _ + 1)   
        ...
        for episode_index in tqdm(range(num_runs_displayed), desc="Processing episodes"):
            ...
            # Use consistent seed (0) as base to ensure reproducibility across runs
            episode_seed = 0 + episode_index
            ...
            while not terminated and gymenv.current_episode_step <= 8000:
                with torch.no_grad():
                    # Convert observation to GPU tensor if CUDA is available
                    if torch.cuda.is_available():
